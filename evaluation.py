...
def evaluate_batch_bayesian(model, x, y, mc_sample_size=50, correct_mask=None, non_correct_mask=None,
    ...
    # results = _eval_bayesian(model, x, y, mc_sample_size, correct_mask, non_correct_mask, logger)
    results = _eval_bayesian(model, x, y, mc_sample_size)

    torch.cuda.empty_cache()
    return results


def seceval(model, x, y, attack, epsilon_max, epsilon_step, attack_iterations, mc_sample_size_during_evaluation=100,
            device='cpu'):
