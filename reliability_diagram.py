...
def compute_calibration(true_labels, pred_labels, confidences, num_bins=10):
    ...
    bin_accuracies = np.zeros(num_bins, dtype=np.float16)
    bin_confidences = np.zeros(num_bins, dtype=np.float16)
    bin_counts = np.zeros(num_bins, dtype=np.int16)
...
def _reliability_diagram_combined(bin_data, 
    ...
    new_ticks = np.abs(ax[1].get_yticks()).astype(np.int16)
    ax[1].set_yticks(new_ticks)    
